...
def parse_text(txt_path):
    """
    CIS Benchmark parser that:
    - Only parses controls after 'Recommendations'
    - Anchors on 'Profile Applicability'
    - Rolls back up to 4 lines to find control ID and Title
    - Handles ID+Title on one line or on consecutive lines
    - Extracts all fields using section header mapping logic
    """
    section_names = [
        "Profile Applicability", "Applicability", "Description", "Setting Description",
        "Rationale", "Rationale Statement", "Impact", "Impact Statement",
    ...
    for idx in range(len(prof_app_indices)-1):
        ...
        # Roll back up to 4 lines to find control id/title
        header_line = None
        for rollback in range(1, 5):
            ...
            if m:
                ...
                break
            # Case where ID is on its own line, title on the next
            ...
            if m2 and candidate_idx+1 < pa_idx:
                ...
                break
        if not header_line:
            continue  # Can't find header for this block; skip
        ...
        while idx2 < len(block):
            ...
            if is_header:
                if current_field and field_buffer:
                    canon_field = header_map.get(current_field.lower(), current_field.title())
                    control[canon_field] = " ".join(field_buffer).strip()
                canon_field = header_map.get(header_candidate, header_candidate.title())
                current_field = canon_field
                field_buffer = []
            else:
